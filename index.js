...
async function getNextEssayTopic(email) {
  ...
  // If already sent today, do not send again
  if (
...
app.get('/signup', (req, res) => {
  res.render('signup');   // matches signup.ejs
});
...
app.get('/dashboard', checkAuth, async (req, res) => {
  ...
  try {
    ...
    let streak = 0;
    let totalDays = activity.rows.length;
    ...
    let frequency = totalDays / 7;
    let totalHours = activity.rows.reduce((sum, r) => sum + (r.study_hours || 0), 0);
    let intensity = totalHours / (totalDays || 1);
    let topics = activity.rows.reduce((sum, r) => sum + (r.topics_completed || 0), 0);

    let score = Math.round(
      (streak * 2 + frequency * 2 + intensity + topics / 10)
    );
...
async function getMainsData() {
  ...
  result.rows.forEach((row) => {
    ...
    const subject = row.section_name;
...
// GET saved syllabus progress for logged-in user
...
app.delete('/api/study-planner/tasks/:id', checkAuth, async (req, res) => {
  ...
  try {
    await pool.query(
      `DELETE FROM study_planner_tasks WHERE id = $1 AND email = $2`,
      [id, email]
    );
...
const standardBooksData = [
  {subject: 'History',title: 'Modern India (Bipin Chandra Old NCERT)',author: 'Bipin Chandra',link: '/pdfs/standard_books/modern_india_bipin_chandra_old_ncert.pdf'},
  {subject: 'History',title: 'Ancient India',author: 'RS Sharma',link: '/pdfs/standard_books/ancient_india_rs_sharma.pdf'}
...
console.log("GNEWS_API_KEY length:", process.env.GNEWS_API_KEY ? process.env.GNEWS_API_KEY.length : 0);
...
async function fetchCurrentAffairs() {
  try {
    ...
    for (const item of articles) {
      ...
      let analysis = generateAnalysis(item);
...
app.post('/save-notification-preference', checkAuth, async (req, res) => {
  ...
  try {
    ...
    // If user clicked YES, send first mail immediately
    if (enableNotifications === true || enableNotifications === "true") {
      ...
      // If already sent today, do not advance again
      if (
        user?.last_essay_sent_date &&
        new Date(user.last_essay_sent_date).toISOString().split("T")[0] === today &&
